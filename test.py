...
def perspective_test(M, image_original, image_transformed, feature_image_original):
    homography_inverse = np.linalg.inv(M)
    # homography_inverse = homography_inverse / homography_inverse[2, 2]  # Normalize the inverse homography
    feature_image_transformed = get_feature_coordinates_in_transformed_image(homography_inverse, feature_image_original)
    image_original_with_feature = draw_feature_in_image(feature_image_original, image_original)
    image_transformed_with_feature = draw_feature_in_image(feature_image_transformed, image_transformed)
...
def transform_feature_coordinate(image_original, image_transformed, feature_image_original, homography_matrix):
